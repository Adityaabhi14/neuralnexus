...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    if (!name || !email || !password) return setError('Complete all parameters.');
    setLoading(true);
    setError('');
    try {
  ...
  useEffect(() => {
    ...
    if (window.google) {
      ...
      const interval = setInterval(() => {
        if (window.google) { clearInterval(interval); initGoogleSignIn(); }
      }, 100);
  ...
  return (
    <div className="auth-wrapper">
      <div className="auth-box">
        ...
        <p style={{ textAlign: 'center', color: 'var(--text-muted)', marginBottom: '32px', fontSize: '14px' }}>Create your account</p>
        
        {error && <div className="alert-error">{error}</div>}
        
        <form onSubmit={handleRegister}>
          <div className="form-group">
            <input type="text" className="form-control" placeholder="Display Name" value={name} onChange={e=>setName(e.target.value)} />
          </div>
          <div className="form-group">
